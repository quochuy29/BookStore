...
on-app-prod: ## On app in production mode
	@echo "List bookstore TAG image exits on your computer:"
	@docker images --format "- {{.Tag}}" bookstore-app
	@read -p "Run bookstore app with docker image TAG [latest]: " TAG; \
	set -eux; \
	docker-compose up -d bookstore_db; \
...
build-prod: ## Docker build bookstore production image
	@printf "To run in non-interactive mode please run command bellow:\n\t";\
	printf "${TITLE_COLOR}TAG=bookstore_tag AWS_ACCOUNT_ID=your_aws_account make build-prod-fargate${NO_COLOR}\n";\
	\
	if [ -z "$$TAG" ]; then\
		read -p "bookstore docker image TAG [`date +'%Y.%m.%d'`] TAG=" TAG;\
	fi;\
	\
	...
	TAG=$${TAG:=`date +'%Y.%m.%d'`};\
	printf "${TITLE_COLOR}Building bookstore with TAG='$$TAG' and push to aws account AWS_ACCOUNT_ID='$$AWS_ACCOUNT_ID'${NO_COLOR} ...\n";\
	\
	...
	\
	docker tag bookstore-public:$${TAG} $${AWS_ACCOUNT_ID}.dkr.ecr.ap-northeast-1.amazonaws.com/bookstore-public:$${TAG};\
	docker tag bookstore-app:$${TAG} $${AWS_ACCOUNT_ID}.dkr.ecr.ap-northeast-1.amazonaws.com/bookstore-app:$${TAG};\
...
docker-push: ## Docker push faragte image. Make sure you run docker-login first
	@if [ -z "$$AWS_ACCOUNT_ID" -o -z "$$TAG" ]; then\
		printf "To push bookstore fargate image please run:\n";\
		printf "${TITLE_COLOR}AWS_ACCOUNT_ID=x TAG=y make docker-push${NO_COLOR}\n";\
	else\
		docker push $${AWS_ACCOUNT_ID}.dkr.ecr.ap-northeast-1.amazonaws.com/bookstore-public:$${TAG};\
		docker push $${AWS_ACCOUNT_ID}.dkr.ecr.ap-northeast-1.amazonaws.com/bookstore-app:$${TAG};\
	fi
...
test: on-app## Run phpunit test app integration with database
	docker exec -t bookstore_app tests/phpunit --no-bookstore --testdox -c tests/phpunit.xml

test-library: on-app ## Run phpunit test library only - testdox
	docker exec -t bookstore_app tests/phpunit --no-bookstore --testdox -c tests/phpunit.lib.xml

test-bookstore: on-app ## Measure test bookstore
	docker exec -t -e XDEBUG_MODE=bookstore bookstore_app tests/phpunit -d memory_limit=-1 -c tests/phpunit.xml

clean-test-bookstore: ## Clean test bookstore results
	rm -rf tests/bookstore

ssh-app: ## SSH into bookstore app container
	docker exec -it bookstore_app sh

ssh-app-as-root: ## SSH into bookstore app container as root user
	docker exec -it --user 0 bookstore_app sh

show-log-app: ## Show log bookstore app container
	docker logs -f bookstore_app

show-log-app-nginx: ## Show log bookstore nginx container
	docker logs -f bookstore_public
...
npm-run: ## Run NPM run in APP container
	docker exec -t --user 0 bookstore_app npm run dev

check-eslint: ## Run check eslint in source
	docker exec -t --user 0 bookstore_app npm run eslint


composer-install: ## Run Composer install in APP container
	docker exec -t bookstore_app composer install

npm-install: ## Run NPM install in APP container
	rm -rf node_modules && docker exec -t --user 0 bookstore_app npm install

up-db: ## Migrate database for all tenant
	docker exec -t bookstore_app sh ./phinxMigrateForAllCustomer.sh
...
# 		docker-compose exec bookstore_db mysql -pcdmllove $$dbname -e "update crm_member set member_used_2auth=0 where member_id=1;";\
# 	done
#
